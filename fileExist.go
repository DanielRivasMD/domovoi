...
// FileExist checks if a file exists. If it doesn't, it executes the provided
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			if notFoundAction != nil {
				...
				if actionErr != nil {
					return false, horus.NewHerror("check file", fmt.Sprintf("'%s' not found action failed", filePath), actionErr, map[string]any{"path": filePath})
				}
				return ok, nil
			}
			return false, horus.NewHerror("check file", fmt.Sprintf("file '%s' not found", filePath), nil, map[string]any{"path": filePath, "action": "none"})
		}
		return false, horus.NewHerror("check file", fmt.Sprintf("error checking file '%s'", filePath), err, map[string]any{"path": filePath})
	}
	if verbose {
